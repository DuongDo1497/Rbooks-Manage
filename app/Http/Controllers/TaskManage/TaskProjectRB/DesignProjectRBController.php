...
use RBooks\Services\DesignProjectRBService;
...
class DesignProjectRBController extends Controller
{
    public function __construct(DesignProjectRBService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_rb.design.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 3)->where('module_type', 11)->where('roman_numerals', 'I');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 3)->where('module_type', 11);
        // Setup title
