...
use RBooks\Services\MktEventService;
use RBooks\Services\EmployeeService;
use App\Http\Controllers\TaskManage\TaskOne\TaskOneController;
use \Auth;

class MktEventController extends Controller
{
    public function __construct(MktEventService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_event.marketing.');
    ...
    {   
        ...
        $this->view->collections = $this->main_service->getAll()->where('module_id', 6)->where('module_type', 13);
        // Setup title
        $this->view->setSubHeading('Plan Event');
