...
use RBooks\Services\WritingOtherService;
use RBooks\Services\EmployeeService;
use RBooks\Repositories\TaskChildRepository;
use RBooks\Repositories\TaskOneRepository;
use \Auth;

class WritingOtherController extends Controller
{
    public function __construct(WritingOtherService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.taskothers.writing.');
    ...
    {   
        ...
        $this->view->collections = $this->main_service->getAll()->where('module_id', 10)->where('module_type', 10);
        // Setup title
    ...
    {
        $this->view->taskWriting = $this->main_service->find($id);
