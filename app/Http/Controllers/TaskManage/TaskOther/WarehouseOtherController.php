...
use RBooks\Services\WarehouseOtherService;
use RBooks\Services\EmployeeService;
use RBooks\Repositories\TaskChildRepository;
use RBooks\Repositories\TaskOneRepository;
use \Auth;

class WarehouseOtherController extends Controller
{
    public function __construct(WarehouseOtherService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.taskothers.warehouse.');
    ...
    {   
        ...
        $this->view->collections = $this->main_service->getAll()->where('module_id', 17)->where('module_type', 10);
        // Setup title
        $this->view->setSubHeading('Khác');

        if(Auth::user()->employee()->first()->division()->first()->whereIn('code_division', ['DATA', 'SALES', 'VANHANH', 'KETOAN']) || Auth::user()->roles()->first()->name == 'owner') {
