...
use RBooks\Services\ContentOtherService;
...
{
    public function __construct(ContentOtherService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.taskothers.content.');
    ...
    {   
        ...
        $this->view->collections = $this->main_service->getAll()->where('module_id', 1)->where('module_type', 10);
        // Setup title
        $this->view->setSubHeading('Khác');

        if(Auth::user()->employee()->first()->whereIn('id', [14, 15]) || Auth::user()->roles()->first()->name == 'owner') {
