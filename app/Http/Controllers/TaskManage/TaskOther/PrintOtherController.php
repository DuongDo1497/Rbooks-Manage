...
use RBooks\Services\PrintOtherService;
use RBooks\Services\EmployeeService;
use RBooks\Repositories\TaskChildRepository;
use RBooks\Repositories\TaskOneRepository;
use \Auth;

class PrintOtherController extends Controller
{
    public function __construct(PrintOtherService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.taskothers.print.');
    ...
    {   
        ...
        $this->view->collections = $this->main_service->getAll()->where('module_id', 13)->where('module_type', 10);
        // Setup title
        $this->view->setSubHeading('Khác');

        if(Auth::user()->employee()->first()->division()->first()->code_division == 'VANHANH' || Auth::user()->id == '245' || Auth::user()->roles()->first()->name == 'owner') {
