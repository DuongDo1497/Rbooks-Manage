...
use RBooks\Services\HrOtherService;
...
{
    public function __construct(HrOtherService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.taskothers.hr.');
    ...
    {   
        ...
        $this->view->collections = $this->main_service->getAll()->where('module_id', 7)->where('module_type', 10);
        // Setup title
    ...
    {
        $this->view->taskHR = $this->main_service->find($id);
    ...
    public function LeadAssign(Request $request, $id)
    {
        ...
        if(Auth::user()->roles()->first()->name == "owner" || Auth::user()->roles()->first()->name == "Leader" || Auth::user()->id == 231 ) {
