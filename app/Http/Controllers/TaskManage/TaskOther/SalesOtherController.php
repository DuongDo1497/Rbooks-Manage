...
use RBooks\Services\SalesOtherService;
...
{
    public function __construct(SalesOtherService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.taskothers.sales.');
    ...
    {   
        ...
        $this->view->collections = $this->main_service->getAll()->where('module_id', 8)->where('module_type', 10);
        // Setup title
    ...
    public function LeadAssign(Request $request, $id)
    {
        ...
        if(Auth::user()->roles()->first()->name == "owner" || Auth::user()->roles()->first()->name == "Leader" || Auth::user()->id == 235 || Auth::user()->id == 236) {
