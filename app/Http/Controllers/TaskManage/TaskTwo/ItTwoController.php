...
use RBooks\Services\ItTwoService;
use RBooks\Repositories\TaskWaitReceiveRepository;
use App\Constants\NotificationMessage;
use \Auth;

class ItTwoController extends Controller
{
    public function __construct(ItTwoService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.tasktwos.it.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 4)->where('roman_numerals', 'XVI');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 4)->where('module_type', 2);
        // Setup title
