...
class SalesTwoController extends Controller
{
    public function __construct(TaskOneService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.tasktwos.sales.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 8)->where('roman_numerals', 'XXVIII');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 8)->where('module_type', 2);
        // Setup title
    ...
    public function LeadAssign(Request $request, $id)
    {
        ...
        $this->view->employees = app(EmployeeService::class)->getAll()->whereIn('division_id', 8);
