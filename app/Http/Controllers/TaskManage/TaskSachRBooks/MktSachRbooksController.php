...
use RBooks\Services\MktSachRbooksService;
...
class MktSachRbooksController extends Controller
{
    public function __construct(MktSachRbooksService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_sach_rbooks.marketing.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 6)->where('module_type', 16)->where('roman_numerals', 'II');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 6)->where('module_type', 16);
        // Setup title
