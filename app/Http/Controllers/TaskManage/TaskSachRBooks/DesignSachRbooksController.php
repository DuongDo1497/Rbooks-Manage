...
use RBooks\Services\DesignSachRbooksService;
...
class DesignSachRbooksController extends Controller
{
    public function __construct(DesignSachRbooksService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_sach_rbooks.design.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 3)->where('module_type', 16)->where('roman_numerals', 'I');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 3)->where('module_type', 16);
        // Setup title
