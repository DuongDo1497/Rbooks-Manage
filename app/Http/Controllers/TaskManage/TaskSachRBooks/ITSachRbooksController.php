...
class ITSachRbooksController extends Controller
{
    public function __construct(TaskOneService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_sach_rbooks.it.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 4)->where('module_type', 16)->where('roman_numerals', 'IV');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 4)->where('module_type', 16);
        // Setup title
