...
use RBooks\Services\WritingSachRbooksService;
use RBooks\Services\EmployeeService;
use App\Http\Controllers\TaskManage\TaskOne\TaskOneController;
use \Auth;

class WritingSachRbooksController extends Controller
{
    public function __construct(WritingSachRbooksService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_sach_rbooks.writing.');
    ...
    {   
        ...
        $this->view->collections = $this->main_service->getAll()->where('module_id', 10)->where('module_type', 16);
        // Setup title
