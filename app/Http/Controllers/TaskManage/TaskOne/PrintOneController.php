...
use RBooks\Services\PrintOneService;
...
{
    public function __construct(PrintOneService $service)
    {
        ...
        $this->setViewPrefix('task-manage.taskones.printing.');
        $this->setRoutePrefix('prints-');
    ...
    {
        $this->view->collections = $this->main_service->getAll()->where('module_id', 13)->where('module_type', 1);
        // Setup title
    ...
    public function LeadAssign(Request $request, $id)
    {
        ...
        $this->view->setSubHeading('Chi tiết (Leader In ấn giao Task)');
    ...
    {
        $this->view->detailTask = app(TaskOneController::class)->generalDetail($id);
        $this->view->setSubHeading('Chi tiết (Nhân viên In ấn nhận Task)');
    ...
    {
        $this->view->detailTask = app(TaskOneController::class)->generalDetail($id);
        $this->view->setSubHeading('Chi tiết (Leader In ấn duyệt Task)');
        if(Auth::user()->roles()->first()->name == "owner" || Auth::user()->roles()->first()->name == "Leader") {
    ...
    {
        $this->view->detailTask = app(TaskOneController::class)->generalDetail($id);
        $this->view->setSubHeading('Chi tiết (Leader In ấn báo cáo Task CEO)');
        if(Auth::user()->roles()->first()->name == "owner" || Auth::user()->roles()->first()->name == "Leader") {
