...
use RBooks\Services\MarketingOneService;
use RBooks\Repositories\TaskWaitReceiveRepository;
use RBooks\Repositories\EmployeeRepository;
use App\Constants\NotificationMessage;
use \Auth;

class MarketingOneController extends Controller
{
    public function __construct(MarketingOneService $service)
    {
        ...
        $this->setViewPrefix('task-manage.taskones.marketing.');
        $this->setRoutePrefix('marketings-');
    ...
    {   
        $this->view->collections = $this->main_service->getAll()->where('module_id', 6)->where('module_type', 1);
        // Setup title
