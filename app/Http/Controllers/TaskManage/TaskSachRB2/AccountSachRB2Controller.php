...
class AccountSachRB2Controller extends Controller
{
    public function __construct(TaskOneService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_sach_rbooks.account2.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 5)->where('module_type', 17)->where('roman_numerals', 'XXIII');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 5)->where('module_type', 17);
        // Setup title
