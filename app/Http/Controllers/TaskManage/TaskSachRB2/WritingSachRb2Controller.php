...
use RBooks\Services\WritingSachRb2Service;
...
class WritingSachRb2Controller extends Controller
{
    public function __construct(WritingSachRb2Service $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_sach_rbooks.writing2.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 10)->where('module_type', 17)->where('roman_numerals', 'VIII');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 10)->where('module_type', 17);
        // Setup title
