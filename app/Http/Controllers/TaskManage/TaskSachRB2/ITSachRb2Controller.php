...
use RBooks\Services\ITSachRb2Service;
...
class ITSachRb2Controller extends Controller
{
    public function __construct(ITSachRb2Service $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_sach_rbooks.it2.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 4)->where('module_type', 17)->where('roman_numerals', 'XV');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 4)->where('module_type', 17);
        // Setup title
