...
class SalesSachRB2Controller extends Controller
{
    public function __construct(TaskOneService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('task-manage.task_sach_rbooks.sales2.');
    ...
    {   
        ...
        $this->view->taskWaits = app(TaskWaitReceiveRepository::class)->get()->where('module_id', 8)->where('module_type', 17)->where('roman_numerals', 'XXVI');

        $this->view->collections = $this->main_service->getAll()->where('module_id', 8)->where('module_type', 17);
        // Setup title
