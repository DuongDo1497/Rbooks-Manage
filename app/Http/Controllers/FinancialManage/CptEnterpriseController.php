...
use App\Http\Requests\CptEnterpriseStoreRequest;
use RBooks\Services\CptEnterpriseService;
use RBooks\Services\EmployeeService;
use RBooks\Models\CtcpList;
use RBooks\Models\Employee;
use App\Constants\NotificationMessage;

class CptEnterpriseController extends Controller
{
    public function __construct(CptEnterpriseService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('financial-manage.cpt_enterprises.');
        $this->setRoutePrefix('cpt_qldn-');

        $this->view->accounting_employee = Employee::find([1, 2, 6]);
        $this->view->mclists = \RBooks\Models\McList::all()->where('mc_group', 2);

        $this->view->setHeading('home.Chi phí quản lý doanh nghiệp');
    }
    ...
    public function store(CptEnterpriseStoreRequest $request)
    {
    ...
    {
        $this->view->cpt_qldn = $this->main_service->find($id);
    ...
    public function list_cptt_dn(Request $request)
    {
        // Tổng tiền k vat
        $paided_cost_novat = 0;
        foreach($this->main_service->getAll()->where('paided_cost_vat', '!=', 0)->where('cplist_id', 3) as $novat) {
            $paided_cost_novat += $novat['paided_cost_novat'];
        }

        // Tổng tiền có vat
        $paided_cost_vat = 0;
        foreach($this->main_service->getAll()->where('paided_cost_vat', '!=', 0)->where('cplist_id', 3) as $vat) {
        ...
        $this->view->list_cptt_dns = CtcpList::where('paided_cost_vat', '!=', 0)->where('cplist_id', 3)->orderBy('id', 'desc')->where($searchFields, 'like', "%$searchValue%")->paginate(25);
        $this->view->setSubHeading('home.Chi phí thực tế');
        return $this->view('cptt.list_cptt_doanhnghiep');
    }

    public function list_cpcn_dn(Request $request)
    {
        // Tổng tiền k vat
        $remaining_cost_novat = 0;
        foreach($this->main_service->getAll()->where('type_cost', '2')->where('cplist_id', 3) as $novat) {
            $remaining_cost_novat += $novat['remaining_cost_novat'];
        }

        // Tổng tiền có vat
        $remaining_cost_vat = 0;
        foreach($this->main_service->getAll()->where('type_cost', '2')->where('cplist_id', 3) as $vat) {
        ...
        $this->view->list_cpcn_dns = CtcpList::where('type_cost', '2')->where('cplist_id', 3)->orderBy('id', 'desc')->where($searchFields, 'like', "%$searchValue%")->paginate(25);
        $this->view->setSubHeading('home.Công nợ');
        return $this->view('cpcn.list_cpcn_doanhnghiep');
    }
}
