...
use App\Http\Requests\CptCostPriceStoreRequest;
use RBooks\Services\CptCostPriceService;
...
class CptCostPriceController extends Controller
{
    public function __construct(CptCostPriceService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('financial-manage.cpt_costprices.');
        $this->setRoutePrefix('cpt_gvt-');

        $this->view->accounting_employee = Employee::find([1, 2, 6]);
        $this->view->mclists = \RBooks\Models\McList::all()->where('mc_group', 3);

        $this->view->setHeading('home.Chi phí giá vốn tổng');
    }
    ...
    public function index(Request $request )
    {
        ...
        $this->view->collections = app(CtcpList::class)->where('cplist_id', 1)->orderBy('id', 'desc')->where($searchFields, 'like', "%$searchValue%")->paginate($this->view->filter['limit']);
        $this->view->cost = app(CtcpList::class)->where('cplist_id', 1)->get();

        // dd($this->view->filter['limit']);
        // Tổng tiền k vat
        $novat_cost = 0;
        foreach($this->view->cost as $novat) {
            $novat_cost += $novat['novat_cost'];
        }

        // Tổng tiền có vat
        $vat_cost = 0;
        foreach($this->view->cost as $vat) {
    ...
    public function store(CptCostPriceStoreRequest $request)
    {
    ...
    {
        $this->view->cpt_costprice = $this->main_service->find($id);
    ...
    {
        $this->main_service->updateInfo($request, $id);
        return redirect()->back()->with(NotificationMessage::UPDATE_SUCCESS);
    }
    ...
    public function detail($id)
    {
        ...
        $this->view->idcostprice = $id;
        $this->view->setSubHeading('home.Chi tiết');
        return $this->view('detail');
    }

    public function list_cptt_gvhb(Request $request)
    {
        // Tổng tiền k vat
        $paided_cost_novat = 0;
        foreach($this->main_service->getAll()->where('paided_cost_vat', '!=', 0)->where('cplist_id', 1) as $novat) {
            $paided_cost_novat += $novat['paided_cost_novat'];
        }

        // Tổng tiền có vat
        $paided_cost_vat = 0;
        foreach($this->main_service->getAll()->where('paided_cost_vat', '!=', 0)->where('cplist_id', 1) as $vat) {
        ...
        $this->view->list_cptt_gvhbs = CtcpList::where('paided_cost_vat', '!=', 0)->where('cplist_id', 1)->orderBy('id', 'desc')->where($searchFields, 'like', "%$searchValue%")->paginate(25);
        $this->view->setSubHeading('home.Chi phí thực tế');
        return $this->view('cptt.list_cptt_giavon');
    }

    public function list_cpcn_gvhb(Request $request)
    {
        // Tổng tiền k vat
        $remaining_cost_novat = 0;
        foreach($this->main_service->getAll()->where('type_cost', '2')->where('cplist_id', 1) as $novat) {
            $remaining_cost_novat += $novat['remaining_cost_novat'];
        }

        // Tổng tiền có vat
        $remaining_cost_vat = 0;
        foreach($this->main_service->getAll()->where('type_cost', '2')->where('cplist_id', 1) as $vat) {
        ...
        $this->view->list_cpcn_gvhbs = CtcpList::where('type_cost', '2')->where('cplist_id', 1)->orderBy('id', 'desc')->where($searchFields, 'like', "%$searchValue%")->paginate(25);
        $this->view->setSubHeading('home.Công nợ');
        return $this->view('cpcn.list_cpcn_giavon');
    }
}
