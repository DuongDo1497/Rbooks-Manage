...
use App\Http\Requests\CptFixedAssetStoreRequest;
use RBooks\Services\CptFixedAssetService;
use RBooks\Services\EmployeeService;
use RBooks\Models\CtcpList;
use RBooks\Models\Employee;
use App\Constants\NotificationMessage;

class CptFixedAssetController extends Controller
{
    public function __construct(CptFixedAssetService $service)
    {
        parent::__construct($service);

        $this->setViewPrefix('financial-manage.cpt_fixedassets.');
        $this->setRoutePrefix('cpt_tscd-');

        $this->view->accounting_employee = Employee::find([1, 2, 6]);
        $this->view->mclists = \RBooks\Models\McList::all()->where('mc_group', 4);

        $this->view->setHeading('home.Chi phí tổng tài sản cố định');
    }
    ...
    public function store(CptFixedAssetStoreRequest $request)
    {
    ...
    {
        $this->view->cpt_tscd = $this->main_service->find($id);
    ...
    public function list_cptt_tscd(Request $request)
    {
        // Tổng tiền k vat
        $paided_cost_novat = 0;
        foreach($this->main_service->getAll()->where('paided_cost_vat', '!=', 0)->where('cplist_id', 4) as $novat) {
            $paided_cost_novat += $novat['paided_cost_novat'];
        }

        // Tổng tiền có vat
        $paided_cost_vat = 0;
        foreach($this->main_service->getAll()->where('paided_cost_vat', '!=', 0)->where('cplist_id', 4) as $vat) {
        ...
        $this->view->list_cptt_tscds = CtcpList::where('paided_cost_vat', '!=', 0)->where('cplist_id', 4)->orderBy('id', 'desc')->paginate(25);

        $this->view->setSubHeading('home.Chi phí thực tế');
        return $this->view('cptt.list_cptt_taisancodinh');
    }

    public function list_cpcn_tscd(Request $request)
    {
        // Tổng tiền k vat
        $remaining_cost_novat = 0;
        foreach($this->main_service->getAll()->where('type_cost', '2')->where('cplist_id', 4) as $novat) {
            $remaining_cost_novat += $novat['remaining_cost_novat'];
        }

        // Tổng tiền có vat
        $remaining_cost_vat = 0;
        foreach($this->main_service->getAll()->where('type_cost', '2')->where('cplist_id', 4) as $vat) {
        ...
        $this->view->list_cpcn_tscds = CtcpList::where('type_cost', '2')->where('cplist_id', 4)->orderBy('id', 'desc')->paginate(25);
        $this->view->setSubHeading('home.Công nợ');
        return $this->view('cpcn.list_cpcn_taisancodinh');
    }
}
