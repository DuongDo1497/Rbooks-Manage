...
class CptSaleCostService extends BaseService
{
    /**
     * Construct function
     */
    ...
    public function create($request)
    {
        ...
        $cptsalecost = $this->repository->create($data);
        if($request->status == 3 && $paided_cost_vat != 0)
        {
            $insert_paymentslip = [
                'ctcplist_id' => $cptsalecost->id,
    ...
    {
        $cptsalecost = $this->repository->find($id);
        $novat_cost = (removeFormatNumber($request->novat_cost) == '' ? '0' : removeFormatNumber($request->novat_cost)); // Tổng tiền phải chi không vat
        $vat_cost = (removeFormatNumber($request->vat_cost) == '' ? '0' : removeFormatNumber($request->vat_cost)); // Tổng tiền phải chi có vat
        ...
        if($remaining_cost_vat == 0) {
            $code1 = substr($cptsalecost->code, 2);
            $code2 = "PC".$code1;
        } else {
            $code2 = $cptsalecost->code;
        }

        $urlFile = $request->file('file_record');
        if ($urlFile == null) {
            $urlFile = $cptsalecost->file_cost;
        } else {
        ...
        $data = [
            ...
            'creator_cost' => $request->creator_cost,
            'personin_cost' => $request->personin_cost,
            'import_id' => $cptsalecost->import_id,
            'cplist_id' => 2, // Chi phí quản lý bán hàng
            'updated_user_id' => Auth()->user()->id,
        ];
        $cptsalecost = $this->repository->update($data, $id);
        return $cptsalecost;
    }

    public function action($request, $key)
    {
        $cptsalecost = $this->repository->find($key);

        $paided_cost_novat = (removeFormatNumber($request->paided_cost_novat) == '' ? '0' : removeFormatNumber($request->paided_cost_novat)); // Tổng tiền đã trả không vat
        $paided_cost_vat = (removeFormatNumber($request->paided_cost_vat) == '' ? '0' : removeFormatNumber($request->paided_cost_vat)); //Tổng tiền đã trả có vat

        if($cptsalecost->remaining_cost_vat == 0) {
            $status = [
                'type_cost' => 1,
            ];
            ...
            $data = [
                'paided_cost_vat' => $cptsalecost->paided_cost_vat + $paided_cost_vat,
                'paided_cost_novat' => $cptsalecost->paided_cost_novat + $paided_cost_novat,
                'remaining_cost_vat' => $cptsalecost->remaining_cost_vat - $paided_cost_vat,
                'remaining_cost_novat' => $cptsalecost->remaining_cost_novat - $paided_cost_novat,
            ];

            if($cptsalecost->remaining_cost_vat - $paided_cost_vat == 0)
            {
                $code1 = substr($cptsalecost->code, 2);
                $code2 = "PC".$code1;
                $stt = [
    ...
    public function getSortPage($request, $field = 'id', $ctcp = 'desc', $limit = null, $columns = ['*'])
    {
        ...
        $listCtcp = CtcpList::orderBy('id', 'DESC')->where('deleted_at', '=', null)->where('cplist_id', 2)->where($searchFields, 'like', "%$searchValue%");
        return $listCtcp;
