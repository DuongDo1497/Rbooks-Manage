...
class CptOtherService extends BaseService
{
    /**
     * Construct function
     */
    ...
    public function create($request)
    {
        ...
        $cptother = $this->repository->create($data);
        if($request->status == 3 && $paided_cost_vat != 0)
        {
            $insert_paymentslip = [
                'ctcplist_id' => $cptother->id,
                'date_cost' => $request->startday_cost,
                'content' => $request->content,
                'paided_cost_novat' => $request->paided_cost_novat,
                'paided_cost_vat' => $request->paided_cost_vat,
                'note' => $request->note,
                'created_user_id' => Auth()->user()->id,
                'updated_user_id' => Auth()->user()->id,
            ];
    ...
    {
        $cptother = $this->repository->find($id);
        $novat_cost = (removeFormatNumber($request->novat_cost) == '' ? '0' : removeFormatNumber($request->novat_cost)); // Tổng tiền phải chi không vat
        $vat_cost = (removeFormatNumber($request->vat_cost) == '' ? '0' : removeFormatNumber($request->vat_cost)); // Tổng tiền phải chi có vat
        ...
        if($remaining_cost_vat == 0) {
            $code1 = substr($cptother->code, 2);
            $code2 = "PC".$code1;
        } else {
            $code2 = $cptother->code;
        }

        $urlFile = $request->file('file_record');
        if ($urlFile == null) {
            $urlFile = $cptother->file_cost;
        } else {
            $chiphifile = rand(1, 999) . $urlFile->getClientOriginalName();
            $urlFile->move(public_path('fileupload/financial/chiphi/'), $chiphifile);
            $urlFile = 'fileupload/financial/chiphi/' . $chiphifile;
        }
        ...
        $data = [
            ...
            'creator_cost' => $request->creator_cost,
            'personin_cost' => $request->personin_cost,
            'import_id' => $cptother->import_id,
            'cplist_id' => 5, // Chi phí quản lý Khác
            'updated_user_id' => Auth()->user()->id,
        ];
        $cptother = $this->repository->update($data, $id);
        return $cptother;
    }

    public function action($request, $key)
    {
        $cptother = $this->repository->find($key);

        $paided_cost_novat = (removeFormatNumber($request->paided_cost_novat) == '' ? '0' : removeFormatNumber($request->paided_cost_novat)); // Tổng tiền đã trả không vat
        $paided_cost_vat = (removeFormatNumber($request->paided_cost_vat) == '' ? '0' : removeFormatNumber($request->paided_cost_vat)); //Tổng tiền đã trả có vat

        if($cptother->remaining_cost_vat == 0) {
            $status = [
                'type_cost' => 1,
            ];
            ...
            $data = [
            'paided_cost_vat' => $cptother->paided_cost_vat + $paided_cost_vat,
            'paided_cost_novat' => $cptother->paided_cost_novat + $paided_cost_novat,
            'remaining_cost_vat' => $cptother->remaining_cost_vat - $paided_cost_vat,
            'remaining_cost_novat' => $cptother->remaining_cost_novat - $paided_cost_novat,
            ];

            if($cptother->remaining_cost_vat - $paided_cost_vat == 0)
            {
                $code1 = substr($cptother->code, 2);
                $code2 = "PC".$code1;
                $stt = [
    ...
    public function getSortPage($request, $field = 'id', $ctcp = 'desc', $limit = null, $columns = ['*'])
    {
        ...
        $listCtcp = CtcpList::orderBy('id', 'DESC')->where('deleted_at', '=', null)->where('cplist_id', 5)->where($searchFields, 'like', "%$searchValue%");
        return $listCtcp;
