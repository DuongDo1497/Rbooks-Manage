...
class CptFixedAssetService extends BaseService
{
    /**
     * Construct function
     */
    ...
    {
        $cptfixedasset = $this->repository->find($id);
        $novat_cost = (removeFormatNumber($request->novat_cost) == '' ? '0' : removeFormatNumber($request->novat_cost)); // Tổng tiền phải chi không vat
        $vat_cost = (removeFormatNumber($request->vat_cost) == '' ? '0' : removeFormatNumber($request->vat_cost)); // Tổng tiền phải chi có vat
        ...
        if($remaining_cost_vat == 0) {
            $code1 = substr($cptfixedasset->code, 2);
            $code2 = "PC".$code1;
        } else {
            $code2 = $cptfixedasset->code;
        }

        $urlFile = $request->file('file_record');
        if ($urlFile == null) {
            $urlFile = $cptfixedasset->file_cost;
        } else {
        ...
        $data = [
            ...
            'creator_cost' => $request->creator_cost,
            'personin_cost' => $request->personin_cost,
            'import_id' => $cptfixedasset->import_id,
            'cplist_id' => 4, // Chi phí quản lý TSCĐ
            'updated_user_id' => Auth()->user()->id,
        ];
        $cptfixedasset = $this->repository->update($data, $id);
        return $cptfixedasset;
    }
    ...
    public function getSortPage($request, $field = 'id', $ctcp = 'desc', $limit = null, $columns = ['*'])
    {
        ...
        $listCtcp = CtcpList::orderBy('id', 'DESC')->where('deleted_at', '=', null)->where('cplist_id', 4)->where($searchFields, 'like', "%$searchValue%");
        return $listCtcp;
