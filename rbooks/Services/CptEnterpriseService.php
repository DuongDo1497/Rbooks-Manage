...
class CptEnterpriseService extends BaseService
{
    /**
     * Construct function
     */
    ...
    public function create($request)
    {
        ...
        $data = [
            ...
            'cplist_id' => 3, // Chi phí doanh nghiệp
    ...
    {
        $cptenterprise = $this->repository->find($id);
        $novat_cost = (removeFormatNumber($request->novat_cost) == '' ? '0' : removeFormatNumber($request->novat_cost)); // Tổng tiền phải chi không vat
        $vat_cost = (removeFormatNumber($request->vat_cost) == '' ? '0' : removeFormatNumber($request->vat_cost)); // Tổng tiền phải chi có vat
        ...
        if($remaining_cost_vat == 0) {
            $code1 = substr($cptenterprise->code, 2);
            $code2 = "PC".$code1;
        } else {
            $code2 = $cptenterprise->code;
        }

        $urlFile = $request->file('file_record');
        if ($urlFile == null) {
            $urlFile = $cptenterprise->file_cost;
        } else {
        ...
        $data = [
            ...
            'creator_cost' => $request->creator_cost,
            'personin_cost' => $request->personin_cost,
            'import_id' => $cptenterprise->import_id,
            'cplist_id' => 3, // Chi phí quản lý bán hàng
            'updated_user_id' => Auth()->user()->id,
        ];
        $cptenterprise = $this->repository->update($data, $id);
        return $cptenterprise;
    }
    ...
    public function getSortPage($request, $field = 'id', $ctcp = 'desc', $limit = null, $columns = ['*'])
    {
        ...
        $listCtcp = CtcpList::orderBy('id', 'DESC')->where('deleted_at', '=', null)->where('cplist_id', 3)->where($searchFields, 'like', "%$searchValue%");
        return $listCtcp;
