    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">{{ trans('home.Danh sách khen thưởng / kỷ luật') }}</h3>
                <div class="box-tools">
                    <a href="{{ route('disciplines-add', ['employeeid' => $employeeid]) }}" class="btn btn-default"><i class="fa fa-plus" aria-hidden="true"></i> {{ trans('home.Tạo mới') }}</a>
                </div>
            </div>
                ...
                <table class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th width="8%" style="text-align: center;">{{ trans('home.Ngày') }}</th>
                            <th width="10%" style="text-align: center;">{{ trans('home.Loại') }}</th>
                            <th width="12%" style="text-align: center;">{{ trans('home.Số quyết định') }}</th>
                            <th width="20%" style="text-align: center;">{{ trans('home.Nội dung') }}</th>
                            <th width="15%" style="text-align: center;">{{ trans('home.Hình thức') }}</th>
                    ...
                    <tbody>
                        ...
                        @foreach($collections as $model)
                            <tr>
                                ...
                                <td style="text-align: left;">{{ $disciplinetype[$model->checktype_id] }}</td>
                                <td style="text-align: left;">{{ $model->disciplinenumber }}</td>
                                <td style="text-align: left;">{{ $model->contentdiscipline }}</td>
                                <td style="text-align: left;">{{ $model->formdiscipline }}</td>
                                ...
                                <td style="text-align: center;">
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            <li><a href="{{ route('disciplines-edit', ['employeeid' => $employeeid, 'id'=> $model->id]) }}"><i class="fas fa-pencil-alt" style="margin-right: 10px;"></i> {{ trans('home.Chỉnh sửa') }}</a></li>
                                            <li><a href="javascript:void(0)" data-id="{{ $model->id }}" class="btn-delete"><i class="fa fa-trash" aria-hidden="true"></i> {{ trans('home.Xóa') }}</a>
                                                <form style="margin: 0;" name="form-delete-{{ $model->id }}" method="post" action="{{ route('disciplines-delete', ['employeeid' => $employeeid, 'id' => $model->id]) }}">
                                                    {{ csrf_field() }}
                                                    {{ method_field('delete') }}
                                                </form>
                                            </li>
