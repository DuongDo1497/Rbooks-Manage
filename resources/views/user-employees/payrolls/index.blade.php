    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">{{ trans('home.Quá trình lương & phụ cấp nhân viên') }}</h3>
                <div class="box-tools">
                    <a href="{{ route('payrolls-add', ['employeeid' => $employeeid]) }}" class="btn btn-default"><i class="fa fa-plus" aria-hidden="true"></i> {{ trans('home.Tạo mới') }}</a>
                </div>
            </div>
                ...
                <table class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th width="18%" style="text-align: center;">{{ trans('home.Mức lương công việc') }}</th>
                            <th width="18%" style="text-align: center;">{{ trans('home.Phụ cấp') }}</th>
                    ...
                    <tbody>
                        ...
                        @foreach($collections as $model)
                            <tr>
                                ...
                                <td style="text-align: right;">{{ formatNumber($model->worksalary, 1, 0, 0) }}</td>
                                <td style="text-align: right;">{{ formatNumber($model->extrasalary, 1, 0, 0) }}</td>
                                ...
                                <td style="text-align: center;">
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            <li><a href="{{ route('payrolls-edit', ['employeeid' => $employeeid, 'id'=> $model->id]) }}"><i class="fas fa-pencil-alt" style="margin-right: 10px;"></i> {{ trans('home.Chỉnh sửa') }}</a></li>
                                            <li><a href="javascript:void(0)" data-id="{{ $model->id }}" class="btn-delete"><i class="fa fa-trash" aria-hidden="true"></i> {{ trans('home.Xóa') }}</a>
                                                <form style="margin: 0;" name="form-delete-{{ $model->id }}" method="post" action="{{ route('payrolls-delete', ['employeeid' => $employeeid, 'id' => $model->id]) }}">
                                                    {{ csrf_field() }}
                                                    {{ method_field('delete') }}
                                                </form>
                                            </li>
