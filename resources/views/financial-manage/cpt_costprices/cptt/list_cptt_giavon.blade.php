...
@include('financial-manage.cpt_costprices.cptt.search_cptt_giavon')
    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">
                    {{ trans('home.Danh sách chi phí giá vốn hàng bán') }}
                ...
                <div class="box-tools">
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                @foreach($mclists as $mclist)
                                    <li><a href="https://rbooks.vn/public/rbooks-vn-management/public/cpt_gvt/list_cptt_gvhb/?searchValue={{$mclist->id}}&searchFields=itemcost_id">{{ $mclist->name }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                    <div class="btn-group btn-group-sm">
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="{{ route('cptt_gvhb-index', filter_data($filter, 'limit', 10)) }}">10 {{ trans('home.dòng / trang') }}</a></li>
                                <li><a href="{{ route('cptt_gvhb-index', filter_data($filter, 'limit', 25)) }}">25 {{ trans('home.dòng / trang') }}</a></li>
                                <li><a href="{{ route('cptt_gvhb-index', filter_data($filter, 'limit', 50)) }}">50 {{ trans('home.dòng / trang') }}</a></li>
                                <li><a href="{{ route('cptt_gvhb-index', filter_data($filter, 'limit', 100)) }}">100 {{ trans('home.dòng / trang') }}</a></li>
                            </ul>
                        </div>
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="{{ route('cptt_gvhb-index', filter_data($filter, 'sortedBy', 'asc')) }}"><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> {{ trans('home.Tăng dần') }}</a></li>
                                <li><a href="{{ route('cptt_gvhb-index', filter_data($filter, 'sortedBy', 'desc')) }}"><i class="fa fa-sort-amount-desc" aria-hidden="true"></i> {{ trans('home.Giảm dần') }}</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        @php
                            $i = 1
                        @endphp
                        @foreach($list_cptt_gvhbs as $list_cptt_gvhb)
                            <tr>
                                <td>
                                    <label>
                                ...
                                <td style="text-align: center;">{{ $list_cptt_gvhb->code }}</td>
                                <td style="text-align: center;">{{ $list_cptt_gvhb->mclist()->first()->name }}</td>
                                <td style="text-align: center;">{{ $list_cptt_gvhb->startday_cost }}</td>
                                <td style="text-align: center;">{{ $list_cptt_gvhb->supplier_name }}</td>
                                <td>{{ $list_cptt_gvhb->content }}</td>
                                <td>{{ $list_cptt_gvhb->import()->first() == NULL ? "-" : $list_cptt_gvhb->import()->first()->warehouse_import_code }}</td>
                                <td style="text-align: center;">{{ $list_cptt_gvhb->quantity == NULL ? "-" : $list_cptt_gvhb->quantity }}</td>
                                <td style="text-align: center;">{{ number_format($list_cptt_gvhb->paided_cost_novat) }}</td>
                                <td style="text-align: center;">{{ number_format($list_cptt_gvhb->paided_cost_vat) }}</td>
                                <td style="text-align: center;">{{ $list_cptt_gvhb->method_cost == 1 ? "Tiền mặt" : "Chuyển khoản" }}</td>
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            <li><a href="{{ route('cpt-detail', ['id' => $list_cptt_gvhb->id]) }}"><i class="fa fa-info-circle" style="margin-right: 10px;"></i> {{ trans('home.Chi tiết') }}</a></li>
                                            <li><a href="{{ route('cpt_gvt-edit', ['id' => $list_cptt_gvhb->id]) }}"><i class="fas fa-pencil-alt" style="margin-right: 10px;"></i> {{ trans('home.Chỉnh sửa') }}</a></li>
