...
@include('financial-manage.cpt_enterprises.cptt.search_cptt_doanhnghiep')
    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">
                    {{ trans('home.Danh sách chi phí quản lý doanh nghiệp thực tế') }}
                ...
                <div class="box-tools">
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                @foreach($mclists as $mclist)
                                    <li><a href="https://rbooks.vn/public/rbooks-vn-management/public/cpt_qldn/list_cptt_dn/?searchValue={{$mclist->id}}&searchFields=itemcost_id">{{ $mclist->name }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                    <div class="btn-group btn-group-sm">
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="{{ route('cptt_dn-index', filter_data($filter, 'limit', 10)) }}">10 {{ trans('home.dòng / trang') }}</a></li>
                                <li><a href="{{ route('cptt_dn-index', filter_data($filter, 'limit', 25)) }}">25 {{ trans('home.dòng / trang') }}</a></li>
                                <li><a href="{{ route('cptt_dn-index', filter_data($filter, 'limit', 50)) }}">50 {{ trans('home.dòng / trang') }}</a></li>
                                <li><a href="{{ route('cptt_dn-index', filter_data($filter, 'limit', 100)) }}">100 {{ trans('home.dòng / trang') }}</a></li>
                            </ul>
                        </div>
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="{{ route('cptt_dn-index', filter_data($filter, 'sortedBy', 'asc')) }}"><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> {{ trans('home.Tăng dần') }}</a></li>
                                <li><a href="{{ route('cptt_dn-index', filter_data($filter, 'sortedBy', 'desc')) }}"><i class="fa fa-sort-amount-desc" aria-hidden="true"></i> {{ trans('home.Giảm dần') }}</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        @php
                            $i = 1
                        @endphp
                        @foreach($list_cptt_dns as $list_cptt_dn)
                            <tr>
                                <td>
                                    <label>
                                ...
                                <td style="text-align: center;">{{ $list_cptt_dn->code }}</td>
                                <td style="text-align: center;">{{ $list_cptt_dn->mclist()->first()->name }}</td>
                                <td style="text-align: center;">{{ date("d/m/Y", strtotime($list_cptt_dn->startday_cost)) }}</td>
                                <td style="text-align: center;">{{ $list_cptt_dn->supplier_name }}</td>
                                <td style="text-align: center;">{{ $list_cptt_dn->content }}</td>
                                <td style="text-align: center;">{{ number_format($list_cptt_dn->paided_cost_novat) }}</td>
                                <td style="text-align: center;">{{ number_format($list_cptt_dn->paided_cost_vat) }}</td>
                                <td style="text-align: center;">{{ $list_cptt_dn->method_cost == '2' ? 'Chuyển khoản' : 'Tiền mặt' }}</td>
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            <li><a href="{{ route('cpt_qldn-detail', ['id' => $list_cptt_dn->id]) }}"><i class="fa fa-info-circle" style="margin-right: 10px;"></i> {{ trans('home.Chi tiết') }}</a></li>
                                            <li><a href="{{ route('cpt_qldn-edit', ['id' => $list_cptt_dn->id]) }}"><i class="fas fa-pencil-alt" style="margin-right: 10px;"></i> {{ trans('home.Chỉnh sửa') }}</a></li>
