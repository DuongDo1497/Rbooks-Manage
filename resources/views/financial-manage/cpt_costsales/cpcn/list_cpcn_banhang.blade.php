...
@include('financial-manage.cpt_costsales.cpcn.search_cpcn_banhang')
    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">
                    {{ trans('home.Danh sách công nợ bán hàng') }}
                ...
                <div class="box-tools">
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                @foreach($mclists as $mclist)
                                    <li><a href="https://rbooks.vn/public/rbooks-vn-management/public/cpt_qlbh/list_cpcn_bh/?searchValue={{$mclist->id}}&searchFields=itemcost_id">{{ $mclist->name }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                    <div class="btn-group btn-group-sm">
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="{{ route('cpcn_bh-index', filter_data($filter, 'limit', 10)) }}">10 {{ trans('home.dòng / trang') }}</a></li>
                                <li><a href="{{ route('cpcn_bh-index', filter_data($filter, 'limit', 25)) }}">25 {{ trans('home.dòng / trang') }}</a></li>
                                <li><a href="{{ route('cpcn_bh-index', filter_data($filter, 'limit', 50)) }}">50 {{ trans('home.dòng / trang') }}</a></li>
                                <li><a href="{{ route('cpcn_bh-index', filter_data($filter, 'limit', 100)) }}">100 {{ trans('home.dòng / trang') }}</a></li>
                            </ul>
                        </div>
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="{{ route('cpcn_bh-index', filter_data($filter, 'sortedBy', 'asc')) }}"><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> {{ trans('home.Tăng dần') }}</a></li>
                                <li><a href="{{ route('cpcn_bh-index', filter_data($filter, 'sortedBy', 'desc')) }}"><i class="fa fa-sort-amount-desc" aria-hidden="true"></i> {{ trans('home.Giảm dần') }}</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        @php
                            $i = 1
                        @endphp
                        @foreach($list_cpcn_bhs as $list_cptt_bh)
                            <tr>
                                <td>
                                    <label>
                                ...
                                <td style="text-align: center;">{{ number_format($list_cptt_bh->remaining_cost_novat) }}</td>
                                <td style="text-align: center;">{{ number_format($list_cptt_bh->remaining_cost_vat) }}</td>
                                <td style="text-align: center;"><b class="alert-danger">{{ $list_cptt_bh->type_cost == '2' ? 'Chưa Chi' : 'Đã Chi' }}</b></td>
