<div class="box box-search">
    <form action="{{ route('comments-index') }}">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select id="searchFields" class="form-control select2" data-minimum-results-for-search="Infinity" name="searchFields">
                            <option value="content:like;">{{ trans('home.Tất cả') }}</option>
                            <option value="">{{ trans('home.Nội dung') }}</option>
                            <option value="">{{ trans('home.Duyệt') }}</option>
                            <option value="">{{ trans('home.Chưa duyệt') }}</option>
                            <option value="">{{ trans('home.Độc giả') }}</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div class="box-footer text-right">
            ...
            <a href="{{ route('question-index') }}" class="btn btn-primary btn-delete">{{ trans('home.Xóa') }} form</a>
        </div>
    </form>
</div>