    ...
    <div class="col-md-8">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">{{ trans('home.Tạo mới danh mục') }}</h3>
            </div>
            <form role="form" action="{{ route('categories-store') }}?continue=true" method="post" id="category-form">
                {{ csrf_field() }}
                <div class="box-body">
                    <div class="form-group">
                        <label>{{ trans('home.Tên danh mục') }}</label>
                        <input type="text" class="form-control" placeholder="{{ trans('home.Tên danh mục') }}" name="name" id="name">
                        @if($errors->has('name'))<span class="help-block">{{ $errors->first('name') }}</span>@endif
                    </div>
                    <div class="form-group">
                        <label>{{ trans('home.Tên danh mục tiếng anh') }}</label>
                        <input type="text" class="form-control" placeholder="{{ trans('home.Tên danh mục tiếng anh') }}" name="nameEnglish" id="nameEnglish">
