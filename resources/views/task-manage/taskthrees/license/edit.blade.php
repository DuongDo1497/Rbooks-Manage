...
<div class="row">
    <form role="form" action="{{ route('task-update', ['id' => $taskLicense->id]) }}?taskupdate=true" method="post" id="license_threes-form">
        {{ csrf_field() }}
        {{ method_field('put') }}
        ...
        <div class="col-md-8">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group">
                        <label>Mô tả</label>
                        <textarea class="form-control" placeholder="Nhập mô tả" rows="4" name="description">{{ $taskLicense->description }}</textarea>
                    </div>
                    <div class="form-group">
                        <label>Ghi chú</label>
                        <textarea class="form-control" placeholder="Nhập ghi chú" rows="4" name="note">{{ $taskLicense->note }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <a href="{{ route('license_threes-index') }}" class="btn btn-default btn-cancel" tabindex="6">Trở về</a>
                </div>
            </div>
        </div>
    </form>
...
<script>
    $(function() {
        $('.btn-save').click(function() {
            $('#license_threes-form').submit();
        });

        $('#chk-continue').on('ifChecked', function() {
            $('#license_threes-form').attr('action', '');
        });
