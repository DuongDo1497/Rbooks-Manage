...
<div class="row">
    <form role="form" action="{{ route('task-update', ['id' => $taskDesign->id]) }}?taskupdate=true" method="post" id="design-product-form">
        {{ csrf_field() }}
        {{ method_field('put') }}
...
<script>
    $(function() {
        $('.btn-save').click(function() {
            $('#design-product-form').submit();
        });

        $('#chk-continue').on('ifChecked', function() {
            $('#design-product-form').attr('action', '');
        });
