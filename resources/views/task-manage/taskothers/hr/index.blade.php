...
@endif
@include('task-manage.taskothers.hr.search-form')
<style>
                		...
                		<div class="box">
                			...
                			<div class="box-body">
                				<div class="task-body">
                                    ...
                                    @endif
                                    @foreach($collections->where('status', '>=', 0) as $taskHr)
                					<div class="task-items clearfix">
                						<div class="task-name" title="{{ $taskHr->taskname }}">
                                            @if($taskHr->status < 22)
                                                @if((strtotime($taskHr->todate) - strtotime(Carbon\Carbon::now()->toDateString())) / (60 * 60 * 24) < 0)
                                                    <div class="task-name-item" style="color: gray"><b>{{ $taskHr->taskname }}</b></div>
                                                @elseif((strtotime($taskHr->todate) - strtotime(Carbon\Carbon::now()->toDateString())) / (60 * 60 * 24) <= 2)
                                                    <div class="task-name-item warning"><b>{{ $taskHr->taskname }}</b></div>
                                                @else
                                                    <div class="task-name-item"><b>{{ $taskHr->taskname }}</b></div>
                                                @endif
                                            @else
                                                <div class="task-name-item"><b>{{ $taskHr->taskname }}</b></div>
                                            @endif
                                            <div class="task-name-item">
                                                @if($taskHr->priority == 1) 
                                                    <span class="alert-warning"> Gấp/Quan trọng</span>
                                                @elseif($taskHr->priority == 2)
                                                    <span style="background-color: #F3E2A9"> Không gấp/QT</span>
                                                @elseif($taskHr->priority == 3)
                                                ...
                                                @elseif($taskHr->status == 1 || $taskHr->status == 4)
                                            ...
                                            <ul class="task-name-detail">
                                                <li><b>Loại dự án:</b> {{ $taskHr->tasktype }}</li>
                                                @if($taskHr->status < 22)
                                                    @if((strtotime($taskHr->todate) - strtotime(Carbon\Carbon::now()->toDateString())) / (60 * 60 * 24) < 0)
                                                        <li style="color: gray"><b>{{ trans('home.Thời gian') }}:</b> {{ date("d/m/Y", strtotime($taskHr->fromdate)) }} - {{ date("d/m/Y", strtotime($taskHr->todate)) }} (Đã hết hạn)</li>
                                                    @elseif((strtotime($taskHr->todate) - strtotime(Carbon\Carbon::now()->toDateString())) / (60 * 60 * 24) <= 2)
                                                        <li class="warning"><b>{{ trans('home.Thời gian') }}:</b> {{ date("d/m/Y", strtotime($taskHr->fromdate)) }} - {{ date("d/m/Y", strtotime($taskHr->todate)) }} (Gần hết hạn)</li>
                                                    @else
                                                        <li><b>{{ trans('home.Thời gian') }}:</b> {{ date("d/m/Y", strtotime($taskHr->fromdate)) }} - {{ date("d/m/Y", strtotime($taskHr->todate)) }}</li>
                                                    @endif
                                                @else
                                                    <li><b>{{ trans('home.Thời gian') }}:</b> {{ date("d/m/Y", strtotime($taskHr->fromdate)) }} - {{ date("d/m/Y", strtotime($taskHr->todate)) }}</li>
                                                @endif
                                                <li><b>{{ trans('home.Trạng thái') }}:</b>
                                                    @if($taskHr->status == 0 || $taskHr->status == 3 || $taskHr->status > 4)
                                                        <b style="color: green;"> {{ $statusTask['0'] }}</b>
                                                    @elseif($taskHr->status == 1)
                                                        <b style="color: red;"> {{ $statusTask['1'] }}</b>
                                                    @elseif($taskHr->status == 4)
                                                        <b style="color: red;"> {{ $statusTask['4'] }}</b>
                                                    @endif
                                                </li>
                                                <li><b>Người khởi tạo:</b> {{ $taskHr->user()->first()->name }}</li>
                                                <li><b>Mô tả:</b> {{ $taskHr->description }}</li>
                                                <li><b>Ghi chú:</b> {{ $taskHr->note }}</li>
                                            </ul>
                                        </div>
                                            ...
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    @if(Auth()->user()->roles()->first()->name == 'owner')
                                                        <li><a href="{{ route('UserTranslateCreate', ['id' => $taskHr->id]) }}" class="task-detail" data-title="Chi tiết"><i class="fa fa-info-circle"></i> {{ trans('home.Chi tiết') }}</a></li>
                                                    @elseif($taskHr->status < 22 && (strtotime($taskHr->todate) - strtotime(Carbon\Carbon::now()->toDateString())) / (60 * 60 * 24) < 0)
                                                    ...
                                                    @endif
                                                    @if($taskHr->status == 0)
                                                        <li><a href="{{ route('translate_ones-edit', ['id' => $taskHr->id]) }}" class="task-edit" data-title="Chỉnh sửa"><i class="fa fa-pencil-square-o"></i> {{ trans('home.Chỉnh sửa') }}</a></li>
                                                        <li>
                                                            <a href="javascript:void(0)" data-id="{{ $taskHr->id }}" class="btn-delete"><i class="fa fa-trash" aria-hidden="true"></i> {{ trans('home.Xóa') }}</a>
                                                            <form name="form-delete-{{ $taskHr->id }}" method="post" action="{{ route('task-delete', ['id'=> $taskHr->id]) }}">
                                                                {{ csrf_field() }}
                                                                {{ method_field('delete') }}
                                                            </form>
                                                        </li>
                                                    @elseif($taskHr->status == 0 || $taskHr->status == 1 && Auth()->user()->roles()->first()->name == 'Leader' || $taskHr->status == 3 && Auth()->user()->roles()->first()->name == 'Leader' || Auth()->user()->roles()->first()->name == 'owner')
                                                        <li><a href="{{ route('translate_ones-edit', ['id' => $taskHr->id]) }}" class="task-edit" data-title="Chỉnh sửa"><i class="fa fa-pencil-square-o"></i> {{ trans('home.Chỉnh sửa') }}</a></li>
                                                        <li>
                                                            <a href="javascript:void(0)" data-id="{{ $taskHr->id }}" class="btn-delete"><i class="fa fa-trash" aria-hidden="true"></i> {{ trans('home.Xóa') }}</a>
                                                            <form name="form-delete-{{ $taskHr->id }}" method="post" action="{{ route('task-delete', ['id'=> $taskHr->id]) }}">
                                    ...
                                    <div class="task-items clearfix">
                                        <div class="task-name" title="{{ $taskHr->taskname }}">
                                            ...
                                            <div class="task-name-item">
                                                ...
                                                @endif
                                                &nbsp;
                                                @if($taskHr->status == 3 || $taskHr->status > 5)
                                                    <span class="alert-success"> {{ trans('home.Đã duyệt') }}</span>
                                                @elseif($taskHr->status == 0)
                                            ...
                                            <ul class="task-name-detail">
                                                ...
                                                <li><b>Trạng thái:</b>
                                                    ...
                                                    @elseif($taskHr->status == 1)
                                                        <b style="color: red;"> {{ $statusTask['1'] }}</b>
                                                    @elseif($taskHr->status == 3 || $taskHr->status > 5)
                                        ...
                                        <div class="task-function">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    <li><a href="{{ route('hr_other_leadapprove', ['id' => $taskHr->id]) }}" class="task-detail" data-title="Chi tiết"><i class="fa fa-info-circle"></i> Chi tiết</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                	...
                	<div class="task-manage-box">
                		<div class="box">
                			...
                			<div class="box-body">
                                ...
                                @endif
                                @foreach($collections->where('status', '>=', 3) as $taskHr)
                                    <div class="task-items clearfix">
                                        <div class="task-name" title="{{ $taskHr->taskname }}">
                                            <div class="task-name-item"><b>{{ $taskHr->taskname }}</b></div>
                                            <div class="task-name-item">
                                                ...
                                                @elseif($taskHr->status == 4 || $taskHr->status == 1)
                                                    <span class="alert-danger"> Không duyệt</span>
                                                @elseif($taskHr->status == 3)
                                                    <span class="alert-info"> Đang chờ duyệt</span>
                                                @endif
                                            </div>
                                            <ul class="task-name-detail">
                                        ...
                                        <div class="task-function">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    <li><a href="{{ route('hr_other_ceoapprove', ['id' => $taskHr->id]) }}" class="task-detail" data-title="Chi tiết"><i class="fa fa-info-circle"></i> Chi tiết</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                	...
                	<div class="task-manage-box">
                		<div class="box">
                			...
                			<div class="box-body">
                                ...
                                @endif
                                @foreach($collections->where('status', '>=', 6) as $taskHr)
                                    <div class="task-items clearfix">
                                        <div class="task-name" title="{{ $taskHr->taskname }}">
                                            @if($taskHr->status < 22)
                                                @if((strtotime($taskHr->todate) - strtotime(Carbon\Carbon::now()->toDateString())) / (60 * 60 * 24) < 0)
                                            ...
                                            <ul class="task-name-detail">
                                                ...
                                                @endif
                                                <br>
                                                @foreach($taskHr->detailTasks()->get()->where('status', '>=', 6) as $detailTask)
                                                <li><b>Công việc:</b>
                                        ...
                                        <div class="task-function">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    <li><a href="{{ route('hr_other_leadassign', ['id' => $taskHr->id]) }}" class="task-detail" data-title="Chi tiết"><i class="fa fa-info-circle"></i> Chi tiết</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                	...
                	<div class="task-manage-box">
                		<div class="box">
                            ...
                            <div class="box-body">
                                ...
                                @endif
                                @foreach($collections->where('status', '>=', 8) as $taskHr)
                                    <div class="task-items clearfix">
                                        <div class="task-name" title="{{ $taskHr->taskname }}">
                                            @if($taskHr->status < 22)
                                                @if((strtotime($taskHr->todate) - strtotime(Carbon\Carbon::now()->toDateString())) / (60 * 60 * 24) < 0)
                                            ...
                                            <ul class="task-name-detail">
                                                ...
                                                @endif
                                                <br>
                                                @foreach($taskHr->detailTasks()->get()->where('status', '>=', 8) as $detailTask)
                                                <li><b>Công việc:</b>
                                                    {{$detailTask->detailtaskname}}<br>  
                                                <b>Người thực hiện:</b>
                                                    {{$detailTask->employee()->first()->fullname}}<br>
                                        ...
                                        <div class="task-function">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    <li><a href="{{ route('hr_user_receive', ['id' => $taskHr->id]) }}" class="task-detail" data-title="Chi tiết"><i class="fa fa-info-circle"></i> Chi tiết</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                	...
                	<div class="task-manage-box">
                		<div class="box">
                			...
                			<div class="box-body">
                                ...
                                @endif
                                @foreach($collections->where('status', '>=', 13) as $taskHr)
                                    <div class="task-items clearfix">
                                        <div class="task-name" title="{{ $taskHr->taskname }}">
                                            @if($taskHr->status < 22)
                                                @if((strtotime($taskHr->todate) - strtotime(Carbon\Carbon::now()->toDateString())) / (60 * 60 * 24) < 0)
                                            ...
                                            <ul class="task-name-detail">
                                                ...
                                                @endif
                                                <br>
                                                @foreach($taskHr->detailTasks()->get()->where('status', '>=', 13) as $detailTask)
                                                <li><b>Công việc:</b>
                                                    {{$detailTask->detailtaskname}}<br>  
                                                <b>Người thực hiện:</b>
                                                    {{$detailTask->employee()->first()->fullname}}<br>
                                        ...
                                        <div class="task-function">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    <li><a href="{{ route('hr_lead_approve', ['id' => $taskHr->id]) }}" class="task-detail" data-title="Chi tiết"><i class="fa fa-info-circle"></i> Chi tiết</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                	...
                	<div class="task-manage-box">
                		<div class="box">
                			...
                			<div class="box-body">
                                ...
                                @endif
                                @foreach($collections->where('status', '>=', 15 ) as $taskHr)
                                    <div class="task-items clearfix">
                                        <div class="task-name" title="{{ $taskHr->taskname }}">
                                            @if($taskHr->status < 22)
                                                @if((strtotime($taskHr->todate) - strtotime(Carbon\Carbon::now()->toDateString())) / (60 * 60 * 24) < 0)
                                            ...
                                            <ul class="task-name-detail">
                                                ...
                                                @endif
                                                <br>
                                                @foreach($taskHr->detailTasks()->get()->where('status', '>=', 17) as $detailTask)
                                                <li><b>Công việc:</b>
                                                    {{$detailTask->detailtaskname}}<br>  
                                                <b>Người thực hiện:</b>
                                                    {{$detailTask->employee()->first()->fullname}}<br>
                                        ...
                                        <div class="task-function">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-right">
                                                    <li><a href="{{ route('hr_ceo_approve_report', ['id' => $taskHr->id]) }}" class="task-detail" data-title="Chi tiết"><i class="fa fa-info-circle"></i> Chi tiết</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                @foreach($collections->where('status', '>=', 15) as $taskHr)
                                    <div class="task-items clearfix">
                                        <div class="task-name" title="{{ $taskHr->taskname }}">
                                            ...
                                            <ul class="task-name-detail">
                                                <li><b>Loại dự án:</b> {{ $taskHr->tasktype }}</li>
                                                <li><b>Thời gian:</b> {{ date("d/m/Y", strtotime($taskHr->fromdate)) }} - {{ date("d/m/Y", strtotime($taskHr->todate)) }}</li>
                                                <br>
                                                @foreach($taskHr->detailTasks()->get()->where('status', '>=', 22) as $detailTask)
                                                <li><b>Công việc:</b>
                                                    {{$detailTask->detailtaskname}}<br>  
                                                <b>Người thực hiện:</b>
                                                    {{$detailTask->employee()->first()->fullname}}<br>
...
@include('task-manage.taskothers.hr.add')
@endsection
