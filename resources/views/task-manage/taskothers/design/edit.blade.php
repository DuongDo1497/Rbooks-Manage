...
<div class="row">
    <form role="form" action="{{ route('task-update', ['id' => $taskDesign->id]) }}?taskupdate=true" method="post" id="design_others-form">
        {{ csrf_field() }}
        {{ method_field('put') }}
...
<script>
    $(function() {
        $('.btn-save').click(function() {
            $('#design_others-form').submit();
        });

        $('#chk-continue').on('ifChecked', function() {
            $('#design_others-form').attr('action', '');
        });
