...
<div class="row">
    <form role="form" action="{{ route('task-update', ['id' => $taskSales->id]) }}?taskupdate=true" method="post" id="sales_others-form">
        {{ csrf_field() }}
        {{ method_field('put') }}
        ...
        <div class="col-md-4">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <a href="{{ route('sales_others-index') }}" class="btn btn-default btn-cancel" tabindex="6">Trở về</a>
                </div>
            </div>
        </div>
    </form>
...
<script>
    $(function() {
        $('.btn-save').click(function() {
            $('#sales_others-form').submit();
        });

        $('#chk-continue').on('ifChecked', function() {
            $('#sales_others-form').attr('action', '');
        });
