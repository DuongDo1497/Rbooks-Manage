                    ...
                    @foreach($detailTask->detailTasks()->get() as $detailTaskChild)
                        ...
                        <tr>
                            ...
                            @if($detailTaskChild->status == 8)
                                <td style="text-align: center; color: orange;"> {{ $statusTask['1'] }}</td>
                            @elseif($detailTaskChild->status == 11)
                                <td style="text-align: center; color: orange;"> {{ $statusTask['4'] }}</td>
                            @elseif($detailTaskChild->status == 13 || $detailTaskChild->status == 21)
                                <td style="text-align: center; color: #088A85;"> {{ $statusTask['5'] }}</td>
                            @elseif($detailTaskChild->status == 14 || $detailTaskChild->status == 22)
                                <td style="text-align: center; color: green"> {{ $statusTask['6'] }}</td>
                            @endif
                                ...
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if($detailTaskChild->progress < 100)
                                            <li><a href="#" data-toggle="modal" data-id="{{ $detailTaskChild->id }}" data-target="#getReport" onclick="processReports('frm', '{{ $detailTaskChild->id }}')"><i class="fa fa-area-chart" style="margin-right: 10px;"></i> Tiến độ</a></li>
                                        @endif
                                        ...
                                        @endif
                                        <li><a href="{{ route('ceo-edit-taskchild', ['id' => $detailTaskChild->id]) }}"><i class="fas fa-pencil-alt" style="margin-right: 10px;"></i> Chỉnh sửa</a></li>
