		...
		<div class="box-header with-border">
            ...
            <div class="box-tools">
            	<div class="btn-group btn-group-sm">
                    @if($detailTask->status > 2 && Auth::user()->roles()->first()->name == "Leader" || $detailTask->status > 2 && Auth::user()->roles()->first()->name == "owner")
        	...
        	<table class="table table-hover">
                ...
                <tbody>
                	...
                	@php
                		$i = 1
                	@endphp
                	@foreach($detailTask->detailTasks()->get()->where('status', '>=','1') as $detailTaskChild)
                        ...
                        <tr>
                            ...
                            <td style="text-align: center;">{{ $detailTaskChild->employee()->first() == NULL ? "" : $detailTaskChild->employee()->first()->fullname }}</td>
                                ...
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if(Auth::user()->roles()->first()->name == "Leader" || Auth::user()->roles()->first()->name == "owner")
                                            @include('task-manage.taskones.translate.functionGroups')
                                        @elseif($detailTaskChild->status == 8)
