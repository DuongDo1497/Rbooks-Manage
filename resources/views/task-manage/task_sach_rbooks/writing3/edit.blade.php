...
<div class="row">
    <form role="form" action="{{ route('task-update', ['id' => $taskTranslate->id]) }}?taskupdate=true" method="post" id="writing_sach_rb-3-form">
        {{ csrf_field() }}
        {{ method_field('put') }}
...
<script>
    $(function() {
        $('.btn-save').click(function() {
            $('#writing_sach_rb-3-form').submit();
        });

        $('#chk-continue').on('ifChecked', function() {
            $('#writing_sach_rb-3-form').attr('action', '');
        });
