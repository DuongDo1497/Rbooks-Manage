...
<div class="row">
    <form role="form" action="{{ route('task-update', ['id' => $taskDesign->id]) }}?taskupdate=true" method="post" id="design_twos-form">
        {{ csrf_field() }}
        {{ method_field('put') }}
...
<script>
    $(function() {
        $('.btn-save').click(function() {
            $('#design_twos-form').submit();
        });

        $('#chk-continue').on('ifChecked', function() {
            $('#design_twos-form').attr('action', '');
        });
